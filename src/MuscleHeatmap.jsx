...
export default function MuscleHeatmap({scores={},onSelect,selected,mode="coverage",height=250}) {
  const color=value=>mode==="freshness"
    ? value>=70?"var(--primary)":value>=35?"var(--warn)":"var(--muscle-empty)"
    : mode==="sets"
    ? value>20?"var(--muscle-review)":value>=13?"var(--muscle-high)":value>=6?"var(--muscle-moderate)":value>=1?"var(--muscle-low)":"var(--muscle-empty)"
    : value>=2?"var(--muscle-moderate)":value>=1?"var(--muscle-low)":value>0?"var(--muscle-high)":"var(--muscle-empty)";
  const paint=id=>({
    ...base,
    "data-muscle":id,
    style:{cursor:onSelect?"pointer":"default",fill:color(scores[id]||0),stroke:selected===id?"var(--on-surface)":"var(--muscle-stroke)",strokeWidth:selected===id?2.4:base.strokeWidth},
  });
